...
test2<-flights %>% filter( distance >= 1000) %>%
   group_by(origin) %>% 
...
test1<- srqflights %>% 
  group_by(delay,flight,origin)
summarise()

ggplot(test2, aes(x = reorder(origin, avg_distance), y = avg_distance)) + 
  geom_col(color = "#9ebcda", fill = "#9ebcda")+
  coord_flip()+
  scale_y_continuous(name = "distance", labels = scales::comma)+
